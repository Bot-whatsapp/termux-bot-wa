		...
		case 'daftar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```List Banned``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```List User Premium D N S B O T``` :\n'
         ...
         case 'lirik':
			    ...
			    case 'nsfwblowjob':
					...
					if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					if (!isNsfw) return reply(' *FLASE* ')
						res = await fetchJson(`https://api.vhtear.com/nekohentai&apikey=${VthearApi}`, {method: 'get'})
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
...
// only grup fitur anime
              case 'anime':
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=animehappy&apikey=${VthearApi}`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					anu = await fetchJson(`https://api.vhtear.com/pinterest?query=animesad&apikey=${VthearApi}`, {method: 'get'})
					var bor = JSON.parse(JSON.stringify(anu.result));
					var uto =  bor[Math.floor(Math.random() * bor.length)];
					nye = await getBuffer(uto)
...
//akhir kreator
			...
			case 'indohot':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					client.sendMessage(from, nye, image, { caption: 'Nih!', quoted: mek })
					break
					case 'cogan':
					if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (!isGroup) return reply(mess.only.group)
					...
					setTimeout( () => {
					client.sendMessage(from, '_bye bye anak pungooottt_.....', text) // ur cods
					}, 20000) // 1000 = 5s,
					setTimeout( () => {
					client.sendMessage(from, '_ucapkan selamat tinggal bro_….', text) // ur cods
					}, 10000) // 1000 = 5s,
					setTimeout( () => {
					client.sendMessage(from, '_bismillah kick anak pungott_…', text) // ur cods
					}, 1000) // 1000 = 5s,
					setTimeout( () => {
